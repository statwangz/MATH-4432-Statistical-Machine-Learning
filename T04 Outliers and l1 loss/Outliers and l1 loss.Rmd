...
$$\hat{\beta} = \underset{\mathbf{\beta}}{\operatorname{argmin}} \sum_{i = 1}^{N} (y_i - x_i \beta)^2 = \underset{\mathbf{\beta}}{\operatorname{argmin}} \frac{1}{N}\sum_{i = 1}^{N} (y_i - x_i \beta)^2.$$
.panelset[
...
t_k <- -1
l1_maj <- (t^2 + t_k^2) / 2 / abs(t_k)

ggplot(data = NULL) +
  geom_line(aes(x = t, y = l1, color = "l1 norm"), size = 2) +
  geom_line(aes(x = t, y = l1_maj, color = "Majorizer"), size = 2) +
  geom_vline(xintercept = t_k, linetype = "dashed", size = 2) +
  scale_color_manual(name = NULL,
                     breaks = c("l1 norm", "Majorizer"),
                     values = c("l1 norm" = "red", "Majorizer" = "green")) +
  xlab("t") +
  ylab(NULL) +
  theme(
    text = element_text(size = 20),
