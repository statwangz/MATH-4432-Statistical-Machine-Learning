...
Recall the least squares problem for linear regression
$$\hat{\beta} = \underset{\mathbf{\beta}}{\operatorname{argmin}} \sum_{i = 1}^{N} (y_i - x_i \beta)^2.$$
.panelset[
...
- Now given $\{x_n\}_{n=1}^N$, we have a set of observations $\mathcal{D}=\{(x_n,y_n)\}_{n=1}^N$ with $$y_n=f(x_n) + \epsilon_n,$$ where $\epsilon_n \stackrel{\text { i.i.d. }}{\sim} \mathcal{N}(0,0.1^2)$ is random noise.

```{r panelset = c(source = "Code", output = "Minimize the least square error"),fig.align='center',out.width="40%",message=FALSE}
set.seed(123)
N <- 10 # Sample size
x <- runif(N, 0, 1)
y0 <- x # Ground truth
...
```{r panelset = c(source = "Code", output = "Minimize the least square error"),fig.align='center',out.width="50%",message=FALSE}
set.seed(123)

x_ol <- c(x, 0.9)
y0_ol <- c(y0, 0.9)
y_obs_ol <- c(y_obs, -2) # Add noise, observed data

ggplot(data = NULL) +
  geom_point(aes(x = x, y = y_obs), size = 5) +
  geom_point(aes(x = 0.9, y = -2), color = "red", size = 5) +
  geom_smooth(aes(x = x, y = y_obs), method = "lm", color = "blue", size = 1.5) +
  geom_smooth(aes(x = x_ol, y = y_obs_ol), method = "lm", color = "red", size = 1.5) +
  theme(
    text = element_text(size = 18),
    axis.text.y = element_text(size = 18),
    axis.text.x = element_text(size = 18)
...
$$\mathcal{L}_2 = \sum_{i = 1}^{N} (y_i - x_i \beta)^2 = ||y - X \beta||_2^2, \\
\mathcal{L}_1 = \sum_{i = 1}^{N} |y_i - x_i \beta| = ||y - X \beta||_1.$$
...
$$
\underset{\beta}{\operatorname{minimize}} \left\|(X \beta - y) \odot w^k \right\|_2^2
$$
...
beta_0 <- -3 # To demostrate, we fit beta_0 = -3
beta_1 <- 1 # To demostrate, we fit beta_1 = 1
```

]

.panel[.panel-name[Call lm()]
```{r eval = FALSE}
opar <- par()
par(mfrow = c(2, 2))
...
.panel[.panel-name[Call geom_smooth()]
```{r}
p_list <- list() # Figure list
for (k in c(0:3)) { # Here we only repeat 4 times
  ...
  beta_0 <- coef(fit_l1)[1] # Update intercept
  beta_1 <- coef(fit_l1)[2] # Update slope
  p <- ggplot(df, aes(x = x, y = y, size = weight / 2)) +
    geom_point(shape = 16) +
...
- Slides form ELEC 5470 / IEDA 6100A Convex Optimization, Prof. Daniel P. Palomar, ECE, HKUST
